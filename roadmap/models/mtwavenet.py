...
class ChannelGate1D(nn.Module):
    """Fuses per-branch features of shape (Batch, Branches, Features) via a learned channel gate."""
...
class FourBranchResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(4):
            feats[i] = self.branches[i][1](feats[i])
        feats = self.att_block1(feats)

        for i in range(4):
            feats[i] = self.branches[i][2](feats[i])
        feats = self.att_block2(feats)

        for i in range(4):
            feats[i] = self.branches[i][3](feats[i])
        feats = self.att_block3(feats)

        for i in range(4):
            feats[i] = self.branches[i][4](feats[i])
        feats = self.att_block4(feats)
...
class FourBranchResNet50Fusion(FourBranchResNet50):
    ...
    def forward(self, x):
        ...
        fused_embedding = self.fusion_module(embeddings)
...
class HybridMultiBranch(nn.Module):
    def __init__(self, num_classes=200, pretrained=True, dropout=0.5, freeze_resnet_bn=True, *args, **kwargs):
        ...
        self.resnet_branch = nn.Sequential(*list(resnet.children())[:-2])
