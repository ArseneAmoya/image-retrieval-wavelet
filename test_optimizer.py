...
class DummyWCNN(nn.Module):
    def __init__(self):
        ...
        self.backbone.conv1 = nn.Conv2d(3, 64, kernel_size=3, bias=True)
        self.backbone.layer1 = nn.Linear(64, 10)

        self.lh_backbone = nn.Sequential()
        self.lh_backbone.conv1 = nn.Conv2d(3, 64, kernel_size=3, bias=True)

        self.hl_backbone = nn.Sequential()
        self.hl_backbone.conv1 = nn.Conv2d(3, 64, kernel_size=3, bias=True)

        self.hh_backbone = nn.Sequential()
        self.hh_backbone.conv1 = nn.Conv2d(3, 64, kernel_size=3, bias=True)
...
config_objects = OmegaConf.load("config/optimizer/cub.yaml")
