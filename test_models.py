...
def test_transforms(transform_cfg_path='config/transform/cifar_swt.yaml'):
    # Charge la config
...
def test_retrievalnet_with_wresnet(freeze_bn=False):
    ...
    model_configs = OmegaConf.load('config/model/multidino_hashing_attention_pretrained.yaml')
    getter = Getter()
