...
**Redone at num_queries=1 (2026-08-19), matching the adopted default (section 5c) and the accepted paper's own single-query design.** `mflickr_wavelet_type_ablation_nq1` / `voc_wavelet_type_ablation_nq1`, db4 and bior4.4 only — haar reused from the existing N=1 reference points (MIRFLICKR: `mflickr_num_queries_sb96` N=1 = 0.8460 ep40; VOC: `voc_final_headline` 64-bit = 0.9922 ep45, see section 5d):

| wavelet | MIRFLICKR (best ep) | VOC (best ep) |
|---|---|---|
| haar | 0.8460 (40) | **0.9922** (45) |
| db4 | **0.8468** (50) | 0.9875 (35) |
| bior4.4 | 0.8423 (40) | 0.9893 (35) |

Three different rankings across three comparisons now exist for essentially the same architecture: this project's N=4 study (bior4.4 > haar > db4), the accepted continuous-retrieval paper's N=1 study (haar > db4 > bior4.4), and this N=1 hashing rerun (MIRFLICKR: db4 > haar > bior4.4; VOC: haar > bior4.4 > db4 — VOC alone partially agrees with the accepted paper on haar being best, MIRFLICKR doesn't). Every spread here is small — MIRFLICKR 0.0045, VOC 0.0047 — well inside the measured noise floor (σ≈0.012-0.017), smaller even than the N=4 study's spread. **Reading**: wavelet-family choice does not have a consistent winner across datasets or architecture configurations, and every observed ranking is within noise. This is the honest, defensible conclusion for the paper — report as a robustness/insensitivity result (no wavelet family is reliably better), explicitly reconciled against the accepted paper's opposite-context finding, rather than picking whichever ranking happens to favor one wavelet (see section 9 and `PAPER_WRITING_GUIDE.md` section 5.3).
...
## 8. Narrative arc for the revision (plan de rédaction)

The story has shifted from the rejected submission's version. Original claim (implicit, section-2/3/4b now refute the mechanism): *"cross-attention with learnable queries + orthogonality regularization learns to route each query to a specialized frequency sub-band."* That mechanism does not exist — no attention collapse, no specialization (section 2), magnitude never leaves init scale so selectivity is structurally impossible (section 3), and forcing it externally changes nothing (section 4b). The revision cannot repeat that claim; it has to replace it with what the data actually supports.

**Revised claim**: the multi-band SWT decomposition into 4 unshared DINOv2 branches, combined ensemble-style, beats a parameter-matched single-branch baseline by a real margin (+2.2 to +4.3 points, section 4) — the gain comes from the decomposed *inputs*, not from any learned routing between them. Orthogonality regularization does what it's mechanistically designed to do (near-exact query orthogonality) but buys no measurable mAP (section 1) — reported as a verified-but-inert design choice, not a contribution. `num_queries` and wavelet type are reported as sensitivity/robustness results (R2), not tuned contributions (section 5c's explicit scope decision).

Section-by-section mapping, in submission order:

1. **Introduction** — motivate multi-frequency decomposition + strong pretrained backbones for hashing; state the (narrowed) contribution up front so it matches what section 4-4b actually support, not the original routing claim.
2. **Related work** — deep hashing baselines (proxy-based losses, DPSH/HashNet-family), wavelet/frequency-domain retrieval, DINOv2-based retrieval. This is also where the "resembles an accepted paper" concern (see below) needs to be resolved before the section is final — differentiation has to be explicit here, not left implicit.
3. **Method** — SWT into 4 sub-bands, 4 unshared DINOv2-ViT-S/14 branches, cross-attention bottleneck fusion (`num_queries=1` as the stated default now, with the sensitivity curve in an appendix/ablation table, not a tuned hyperparameter), orthogonality regularizer (stated as a geometric regularizer with a verified mechanistic effect, mAP-neutral).
4. **Experimental setup** — datasets + the paper's own bit-length protocol (Table 1: MIRFLICKR {32,64,128}, VOC {32,64,96}, COCO {32,64}), the reproducibility paragraph pre-registered in section 0 (σ≈0.012–0.017 noise floor, one canonical default-config number cited everywhere).
5. **Results**
   - 5.1 Headline retrieval table (section 5d) — the paper's Table 1 replacement, MIRFLICKR done, VOC needs the epoch-50-vs-best-epoch call resolved for 32/96-bit (section 5d caveat) before it's final, COCO's near-ceiling mAP now explained (any-shared-label relevance + train⊆database, verified not a leak).
   - 5.2 Capacity control vs ViT-B (section 4) — the paper's actual load-bearing result now: multi-band beats single-branch at matched parameter count.
   - 5.3 Orthogonality ablation (section 1) — reported honestly as no significant mAP gain, multi-seed backed (the one claim in this paper allowed "X vs Y" language, and the honest finding is "no difference").
   - 5.4 Mechanistic diagnostics (sections 2-3) — why there's no gain: no collapse, magnitude constrained by design. This is what makes 5.3's null result *explained* rather than just reported, which is what R3 actually asked for.
   - 5.5 Sensitivity analyses (R2): wavelet type (5b) and num_queries (5c), both framed as robustness curves with the noise floor stated once, not as tuned choices.
6. **Discussion / limitations** — single-seed sensitivity studies (5b/5c) need the "directional, not conclusive" framing from section 0; VOC's 32/96-bit early-epoch instability (5d) if unresolved by submission; open questions (section 7).
7. **Conclusion** — restate the narrowed claim: decomposition + ensemble combination works, learned routing doesn't, and the paper says so explicitly rather than papering over sections 2-4b.

## 9. Comparison with the accepted "MBW-DINO" paper (continuous retrieval) — read 2026-08-19

Same authors, same architecture name, near-identical Method section (SWT into LL/LH/HL/HH, 4 unshared DINOv2-ViT-S/14 branches, one fully-learnable global query token, bottleneck cross-attention fusion — Fig. 1/2 of the accepted paper describe exactly the module this project calls `CrossAttentionBottleneckHeadAdvanced`). This is genuinely the same architecture applied to a different problem, not a coincidence — real overlap risk, worth managing deliberately rather than hoping a reviewer doesn't notice.

**Constraint (2026-08-19): the accepted paper will NOT be cited, per the advisors' decision.** This removes the "cite it, differentiate explicitly, reconcile the routing claim by name" strategy below wherever it depends on a citation — struck through / marked accordingly. It does **not** remove the underlying risks, and shifts weight onto the mitigations that don't depend on citing:
- **Text-similarity checkers don't care whether you cite.** iThenticate/Turnitin-style checks (used by many venues, sometimes post-acceptance) flag matching text against any indexed document, cited or not — a match against an *uncited* same-author paper reads worse (looks like concealment) than a match against a cited one (reads as normal self-reference). Not citing makes rewriting the Method section in fully independent wording **more** important, not less — this is now the single highest-priority mitigation, see below.
- **The naming collision (both papers using "MBW-DINO") matters more without a citation to explain it.** With no textual link between the two papers, a reader or indexer who notices the shared name and authors has nothing in-text pointing to a legitimate explanation. Recommend dropping "MBW-DINO" from the revision's own title/prose entirely (revert to the original rejected paper's own title/naming, which never used it) rather than just adding a disambiguation sentence.
- **The routing-vs-ensemble finding stands on its own without naming the other paper.** The revision doesn't need to say "unlike X, we show Y" — it can simply report Y (the fusion mechanism behaves as a content-independent combiner, evidenced by sections 2-3-4b) as a standalone mechanistic finding about *this* architecture. That's arguably cleaner writing regardless of the citation constraint — see the revised guidance below.

**What's actually different (legitimate, defensible):**
- **Task**: accepted paper = continuous/real-valued embedding retrieval (mAP@R, R-PR, mAP@10k, no binarization). This revision = deep supervised **hashing** (binary codes, Hamming distance, bit-length sweep 32-128, a completely different loss family — `HashLoss` + learnable proxies + orthogonality regularizer on hash codes, vs. the accepted paper's ROADMAP mAP-surrogate loss on continuous embeddings).
- **Benchmarks**: accepted paper = CIFAR-100, CUB-200-2011 (single-label-ish, continuous-retrieval literature). This revision = MIRFLICKR-25K, VOC2012, MS-COCO (multi-label, deep-hashing literature's own standard benchmark trio). Different fields' baselines and conventions — legitimate separation, should be stated explicitly rather than left implicit.
- **Contribution framing available to this revision, without naming the other paper**: report the mechanism finding as this paper's own diagnostic result — "we directly diagnose the fusion mechanism (attention entropy, per-band share, query magnitude) rather than inferring its behavior from ablation deltas alone, and find it behaves as a content-independent combiner rather than a content-conditional selector; the gain comes from the decomposed inputs (ensemble effect), not learned selectivity." Still a real, differentiated contribution on its own terms, no citation needed to make the point.

**Where the two papers actually contradict each other — this is the "se cogner le pied" risk, not the general resemblance:**
- The accepted paper's §4.3 ("Effectiveness of the fusion mechanism") states outright: *"The semantic representation, presented mainly in the low-frequency (LL) sub-band, can query the high-frequency detail sub-bands... so that the model extracts only the most useful micro-textures **for the query image**... This deep frequency routing avoids the limitations of simple gating."* The load-bearing part isn't "uniform weights" (uniform weights alone don't disprove a deliberate, informed routing decision that happens to land on balance) — it's the specific claim of **per-image, content-conditional selection that outperforms static/gating combination**. Three things measured together refute exactly that specific claim, not just "uniform on average":
  1. Near-uniform for essentially every image, not just on average — the raw pre-softmax score spread's *max* across the 320-image sample (`diagnostics_attention_2026-08-11.txt`, section 2/3's within-row spread stat) is only 0.336, too small to permit real selectivity even for the single most differentiated image in the sample.
  2. Structurally forced, not chosen — the query is one global learned vector, not computed from the image, and its magnitude is capped at init scale by the training objective (section 3). The uniformity isn't an informed per-image judgment; the mechanism lacks the capacity to be anything else, for any image.
  3. Causally inert — forcing a deviation from uniform (section 4b) doesn't cost performance in the clean, dose-dependent way an actively-optimal "uniform is correct here" decision would predict; the degradation is non-monotonic and inside the seed-to-seed noise band.
  Together, these describe a **static, content-independent combination**, not a weaker or different kind of routing. Both papers use the *same fusion module*, but since the accepted paper won't be named, **the revision states its own finding on its own terms**, without framing it as a correction of anyone: "the mixing weights are uniform for effectively every image, structurally capped by the frozen low-magnitude global query rather than freely chosen, and causally irrelevant to retrieval performance." That sentence is true and defensible standing alone — it doesn't need the other paper's claim as a foil to be worth stating, and saying it this way sidesteps the citation question entirely rather than working around it.
- **Wavelet-family ranking — resolved as "no consistent winner" (2026-08-19), reported without reference to the accepted paper.** The `_nq1` reruns (section 5b) are in: at num_queries=1, MIRFLICKR gives db4 (0.8468) > haar (0.8460) > bior4.4 (0.8423) and VOC gives haar (0.9922) > bior4.4 (0.9893) > db4 (0.9875) — disagreeing with each other and with this project's own earlier N=4 study (bior4.4 best). Every spread is inside the noise floor (MIRFLICKR 0.0045, VOC 0.0047). **Conclusion for the paper, stated on this project's data alone**: no wavelet family is reliably better across datasets; report insensitivity to wavelet choice as the finding. No need to reference the accepted paper's own (also-disagreeing) haar-best result to make this point — the internal MIRFLICKR-vs-VOC disagreement already establishes it.
- **Self-plagiarism mechanics — now the top-priority mitigation, precisely because there's no citation to legitimize any overlap.** The architecture description (SWT decomposition, cross-attention equations, Fig. 1/2-equivalent) must not be copy-adapted from the accepted paper's text, full stop — no citation-based fallback ("we adapt this from [X]") is available to explain a match if a similarity checker finds one. Rewrite the Method section from scratch, in independent wording, ideally by someone describing the architecture without the accepted paper open next to them (paraphrase-from-memory rather than edit-in-place) to avoid unconsciously mirroring its sentence structure.

**Timing: not published yet — LNCS, planned December 2026.** The camera-ready is very likely locked (filename says "camera_ready", already delivered to the publisher) — nothing in *that* paper's text/tables/claims is realistically still editable, and its routing claim (§4.3) stands as-is once it appears. Since it won't be cited (advisors' decision, above), timing mainly matters for one thing: both papers become permanently public within the same few months, so any overlap that exists will be discoverable by anyone who reads both, cited or not — there's no "it'll blow over" window regardless of citation choice.

- **Naming collision — now the main lever left to manage the resemblance, since there's no citation to explain it.** Drop "MBW-DINO" from the revision's own title and prose; use the original rejected paper's own name/title instead (*"A Multi-Frequency Deep Hashing Retrieval with Learnable Proxy and Orthogonal Regularization"* — never used "MBW-DINO"). This project's internal shorthand (this file's own title, `MIRFLICKR_DIAGNOSTIC_PLAN.md`, etc.) can keep using "MBW-DINO" informally — that's fine, it's not user-facing — but the submitted paper's text shouldn't, once the accepted paper is out under that exact name for a different architecture-application with the same authors.

**Concrete to-do before this goes out** (revised for no citation): (1) do not reference the accepted paper anywhere in the revision's text — no "in press" citation, no named comparison; (2) state the routing/ensemble finding as this paper's own standalone result (see the reworded sentence above), not as a correction of anything; (3) report the wavelet-family result as "no consistent winner" using only this project's own MIRFLICKR-vs-VOC disagreement as evidence, already sufficient without the accepted paper's data point; (4) drop "MBW-DINO" from the revision's title/prose, use the original title instead; (5) rewrite the Method section from independent wording (not edited from the accepted paper's text) — this is now the single most important mitigation, since a similarity-checker match against an uncited paper is harder to explain away than one against a cited paper.
