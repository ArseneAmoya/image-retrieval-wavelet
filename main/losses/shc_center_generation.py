"""
Stage 2 de SHC : génération des centres de hachage sémantiques.

CODE TIERS, VENDORISÉ TEL QUEL. Source :
  Li Chen, Rui Liu, Yuxiang Zhou, Xudong Ma, Yong Chen, Dell Zhang.
  "Deep Hashing with Semantic Hash Centers for Image Retrieval", ACM TOIS 2025.
  https://github.com/cc752424640/Deep-Hashing-with-Semantic-Hash-Centers-for-Image-Retrieval

Point d'entrée : GenerateSemanticHashCenters(args, S) -> tenseur (bit, num_classes)
de +/-1. `args` doit porter .code_length, .num_classes, .dataset, .device.
L'étage est entièrement agnostique aux labels : il ne voit que la matrice de
...
produit les résultats publiés :
  - `alpha = torch.full((n_class, n_class), 0)` crée un tenseur int64, donc les
    mises à jour flottantes de la variable duale sont tronquées ;
  - dans l'alpha-step, `alpha[i, j]` utilise un `j` non défini qui fuite de la
    boucle précédente, si bien que seule la dernière colonne est mise à jour.
Les corriger donnerait un algorithme différent de celui des auteurs, et des
chiffres non comparables aux leurs.

PIÈGE À SURVEILLER : les auteurs notent en commentaire que `eta` "a un impact
assez important selon le jeu de données". Avec une matrice S mal calibrée,
l'optimisation ne fait rien et renvoie l'initialisation CSQ inchangée — vos
"centres SHC" seraient alors du Hadamard déguisé, sans le moindre avertissement.
Toujours vérifier que `loss1` décroît dans les logs.
"""
import os.path
import torch
import random
...
# --- MODIFIÉ (seul changement fonctionnel) : le dépôt d'origine codait
# `torch.device("cuda:4")` en dur, ce qui plante sur toute machine ayant moins
# de 5 GPU. On respecte désormais CUDA_VISIBLE_DEVICES / le device par défaut.
